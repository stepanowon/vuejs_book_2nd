...
<div class="modal">
    <div class="form" @keyup.esc="cancelEvent">
        ...
        <div class="form-group">
            <label>이름</label>
            <input type="text" name="name" class="long" v-model="contactlocal.name" 
                ref="name" placeholder="이름을 입력하세요" />
        </div>
        <div class="form-group">
            <label>전화번호</label>
            <input type="text" name="tel" class="long" v-model="contactlocal.tel" 
                placeholder="전화번호를 입력하세요" />
        </div>
        <div class="form-group">
            <label>주 소</label>
            <input type="text" name="address" class="long" v-model="contactlocal.address" 
...
export default {
    ...
    methods : {
        submitEvent : function() {
            if (this.mode == "update") {
                this.$store.dispatch(Constant.UPDATE_CONTACT, { contact: this.contactlocal });
            } else {
                this.$store.dispatch(Constant.ADD_CONTACT, { contact: this.contactlocal });
            }
        },
