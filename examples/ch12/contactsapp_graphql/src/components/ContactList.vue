<template>
    ...
    <table id="list" class="table table-striped table-bordered table-hover">
        ...
        <tbody id="contacts" >
            <tr v-for="contact in contactlist.contacts" :key="contact._id">
                <td>{{contact.name}}</td>
                <td>{{contact.tel}}</td>
                <td>{{contact.address}}</td>
                <td><img class="thumbnail" :src="contact.photo" 
                    @click="editPhoto(contact._id)" /></td>
                <td>
                    <button class="btn btn-primary" 
                        @click="editContact(contact._id)">편집</button>
                    <button class="btn btn-primary" 
                        @click="deleteContact(contact._id)">삭제</button>
                </td>
            </tr>
        </tbody>
    </table>
...
export default {
    ...
    methods : {
        ...
        editContact : function(_id) {
            this.$router.push({ name: 'updatecontact', params : { _id:_id } })
        },
        deleteContact : function(_id) {
            if (confirm("정말로 삭제하시겠습니까?") == true) {
                this.$store.dispatch(Constant.DELETE_CONTACT, {_id:_id});
                this.$router.push({ name: 'contacts' })
            }
        },
        editPhoto : function(_id) {
            this.$router.push({ name: 'updatephoto', params: { _id: _id } })
        }
    }
}
</script>
